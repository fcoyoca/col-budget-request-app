...
public record LookupCategoryItemCreated(Guid Id, string name, string description) : DomainEvent;
    ...
    : INotificationHandler<LookupCategoryItemCreated>
{
    public async Task Handle(LookupCategoryItemCreated notification, CancellationToken cancellationToken)
    {
        ...
        var cacheResponse = new GetLookupCategoryResponse(notification.Id, notification.name, notification.description);
